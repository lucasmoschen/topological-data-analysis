...
disconsider when writing the topologies, that it, all below contain these
subsets. 
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \in A} ~^cO_{\alpha}$. As $^c O \subset ~^cO_{\alpha}$ and
        $^cO_{\alpha}$ is finite, so is $^c O$. We conclude $O$ is open set. 
        
        \item Let $\{O_{i}\}_{1 \le i \le n} \subset \T$. So 
        $^cO = ~^c\left(\bigcap_{1 \le i \le n} O_i\right) = \bigcup_{1 \le i
        \le n} ~^cO_i$, where $^cO_i$ if finite. A finite union of finite sets
        is finite, so $O$ is an open set. 
    \end{enumerate}
    ...
    que não é finito. Portanto a intersecção não é conjunto aberto.     
